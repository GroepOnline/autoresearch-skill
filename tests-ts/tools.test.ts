...
test("decideMetric rejects tests_passed=false", () => {
  const result = decideMetric({ direction: "lower", candidate: 50, best: 100, tests_passed: false });
...
test("decideMetric handles zero best value correctly", () => {
  ...
  const better = decideMetric({ direction: "lower", candidate: -1, best: 0, min_effect_size_pct: 1 });
  assert.equal(better.action, "keep");
  ...
  const better2 = decideMetric({ direction: "higher", candidate: 1, best: 0, min_effect_size_pct: 1 });
  assert.equal(better2.action, "keep");

  const worse2 = decideMetric({ direction: "higher", candidate: -1, best: 0, min_effect_size_pct: 1 });
  assert.equal(worse2.action, "discard");
});

test("decideMetric validates inputs", () => {
  assert.throws(() => decideMetric({ direction: "lower", candidate: NaN, best: 100 }), /finite number/);
  assert.throws(() => decideMetric({ direction: "invalid" as unknown as "lower", candidate: 50, best: 100 }), /lower or higher/);
  assert.throws(() => decideMetric({ direction: "lower", candidate: 50, best: NaN }), /finite number/);
});

test("decideMetric respects min_effect_size_pct threshold", () => {
  // 5% improvement should be kept with 3% threshold
  const result = decideMetric({ direction: "lower", candidate: 95, best: 100, min_effect_size_pct: 3 });
  assert.equal(result.action, "keep");
  assert.ok((result.improvement_pct ?? 0) >= 3);

  // 2% improvement should be discarded with 3% threshold
  const result2 = decideMetric({ direction: "lower", candidate: 98, best: 100, min_effect_size_pct: 3 });
...
test("decideMetric handles higher direction correctly", () => {
  ...
  const better = decideMetric({ direction: "higher", candidate: 110, best: 100, min_effect_size_pct: 3 });
  assert.equal(better.action, "keep");
  assert.ok((better.improvement_pct ?? 0) >= 3);

  const worse = decideMetric({ direction: "higher", candidate: 90, best: 100, min_effect_size_pct: 3 });
  assert.equal(worse.action, "discard");
});
...
test("decideMetric handles negative best with positive candidate", () => {
  ...
  const worse = decideMetric({ direction: "lower", candidate: -90, best: -100, min_effect_size_pct: 1 });
  assert.equal(worse.action, "discard");

  const betterHigher = decideMetric({ direction: "higher", candidate: -90, best: -100, min_effect_size_pct: 1 });
