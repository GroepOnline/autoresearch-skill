...
test("e2e fixture: new -> filled contract -> start -> result -> decision -> dashboard", () => {
  ...
  writeFileSync(join(cwd, "autoresearch.sh"), [
    "#!/usr/bin/env bash",
    "set -euo pipefail",
  ...
  appendFileSync(p.jsonl, [
    JSON.stringify({ type: "result", run: 1, metric: "latency_ms", median: 100, timestamp: "2026-05-08T10:00:00Z", description: "baseline" }),
    JSON.stringify({ type: "decision", run: 1, action: "baseline", reason: "baseline measurement", timestamp: "2026-05-08T10:01:00Z" }),
    JSON.stringify({ type: "result", run: 2, metric: "latency_ms", median: 92, timestamp: "2026-05-08T10:02:00Z", description: "remove redundant parse allocation" }),
    JSON.stringify({ type: "decision", run: 2, action: "keep", reason: "improved above threshold", timestamp: "2026-05-08T10:03:00Z" }),
  ].join("\n") + "\n");
