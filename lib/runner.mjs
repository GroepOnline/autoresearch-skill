...
export async function run(options = {}) {
  ...
  console.log(
    `\u2551   Autoresearch Runner (${providerLabel})${dryRun ? " [DRY RUN]" : ""}`.padEnd(
      59
    ) + "\u2551"
  );
  console.log(
  ...
  while (runCount < maxRuns) {
    ...
    if (scoredSkills.length > 0) {
      ...
      const weights = scoredSkills.map((_, i) =>
        Math.pow(scoredSkills.length - i, 2)
      );
